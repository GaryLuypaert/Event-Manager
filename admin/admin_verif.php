...
$sql = $bdd->prepare("SELECT id_user FROM user WHERE login = :login AND password = :password");
$req = $sql->execute(array("login" => $login, "password" => $password));
$result = $sql->fetchAll(PDO::FETCH_OBJ);

if (count($result) > 0) {
    // echo "Vous êtes admin"; Ne rien mettre avant un header(Location)
    $_SESSION['account_id'] = $result[0]->id_user;
                ...
                <div id="main-admin" class="col-md-12">
                    ...
                    <form action="admin_verif.php" method="POST">
                        <div class="form-group">
                            <input type="text" class="form-control" value="" name="login" placeholder="Login" />
                        </div>
                        <div class="form-group">
